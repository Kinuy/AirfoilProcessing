...
namespace
{
	...
	namespace tk
	{
		...
		size_t spline::find_closest(double x) const
		{
			...
			size_t idx = max(int(it - m_x.begin()) - 1, 0);   // m_x[idx] <= x
			return idx;
		}
		...
		namespace internal
		{
			...
			void band_matrix::lu_decompose()
			{
				...
				for (int i = 0; i < this->dim(); i++) {
					...
					j_max = min((int)(this->dim() - 1), i + this->num_upper());
				...
				for (int k = 0; k < this->dim(); k++) {
					i_max = min((int)(this->dim() - 1), k + this->num_lower());  // num_lower not a mistake!
					for (int i = k + 1; i <= i_max; i++) {
						assert(this->operator()(k, k) != 0.0);
						x = -this->operator()(i, k) / this->operator()(k, k);
						this->operator()(i, k) = -x;                         // assembly part of L
						j_max = min((int)(this->dim() - 1), k + this->num_upper());
